-- =============================================================================
-- Compa — Supabase SQL Schema (con 89 proveedores REALES del registro público)
...
-- =============================================================================
-- 3. TABLA: proveedores — 89 PROVEEDORES REALES del registro público
...
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS oportunidad_id UUID REFERENCES oportunidades(id) ON DELETE SET NULL;
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS conversation_id TEXT;
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS preguntas_generadas JSONB;
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS respuesta_raw JSONB;
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS es_fallback BOOLEAN DEFAULT false;
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS creado_en TIMESTAMPTZ DEFAULT now();
ALTER TABLE decisiones ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT now();
ALTER TABLE decisiones ALTER COLUMN es_fallback SET DEFAULT false;
ALTER TABLE decisiones ALTER COLUMN creado_en SET DEFAULT now();

CREATE INDEX IF NOT EXISTS idx_decisiones_productor_id ON decisiones(productor_id);
CREATE INDEX IF NOT EXISTS idx_decisiones_oportunidad_id ON decisiones(oportunidad_id);
CREATE INDEX IF NOT EXISTS idx_decisiones_conversation_id ON decisiones(conversation_id);

-- =============================================================================
-- 4.1 TABLA: match_oportunidades (persistencia de matches para demo/n8n)
-- =============================================================================
CREATE TABLE IF NOT EXISTS match_oportunidades (
    id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    datos_match    JSONB,
    created_at     TIMESTAMPTZ DEFAULT now(),
    UNIQUE (productor_id, oportunidad_id)
);

CREATE INDEX IF NOT EXISTS idx_match_oportunidades_productor_id ON match_oportunidades(productor_id);
CREATE INDEX IF NOT EXISTS idx_match_oportunidades_oportunidad_id ON match_oportunidades(oportunidad_id);
CREATE INDEX IF NOT EXISTS idx_match_oportunidades_estado ON match_oportunidades(estado);
...
ALTER TABLE match_oportunidades ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "all_access" ON productores;
DROP POLICY IF EXISTS "all_access" ON oportunidades;
DROP POLICY IF EXISTS "all_access" ON proveedores;
DROP POLICY IF EXISTS "all_access" ON decisiones;
DROP POLICY IF EXISTS "all_access" ON match_oportunidades;

CREATE POLICY "all_access" ON productores   FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "all_access" ON oportunidades FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "all_access" ON proveedores   FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "all_access" ON decisiones    FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "all_access" ON match_oportunidades FOR ALL USING (true) WITH CHECK (true);
...
-- =============================================================================
-- 7. SEED: oportunidades (43 — generadas de los rubros reales de los 89 proveedores)
-- =============================================================================
...
('Servicio de Apoyo Operativo para Archivo y Atencion Institucional', 'Ministerio de Hacienda', 16400.00, '2026-07-27', 'Servicios Generales', '80111600', 'https://www.comprasal.gob.sv/libre-gestion/lg-24-2026-mh', 'LG'),

('Suministro y Distribucion de Insumos Varios para Oficinas Regionales', 'Ministerio de Hacienda', 27600.00, '2026-07-28', 'Comercio y Distribución', '53100000', 'https://www.comprasal.gob.sv/libre-gestion/lg-25-2026-mh', 'LG');

-- =============================================================================
-- 8. SEED: proveedores — 89 REALES del Registro de Contratistas jul-sep 2025
...
-- Servicios Generales (rubro de cobertura demo)

('CENTRO DE SERVICIO DOÑO, S.A. DE C.V.', 'Sin clasificar', 'Servicios Generales', 'Servicios generales, apoyo operativo y atencion institucional');

-- =============================================================================
-- 9. MATCHING FUNCTION (SQL nativo como fallback del Code node)
-- =============================================================================
CREATE OR REPLACE FUNCTION matching_oportunidades(p_productor_id UUID, p_limit INTEGER DEFAULT 5)
RETURNS TABLE (
    oportunidad_id    UUID,
    titulo            TEXT,
    institucion       TEXT,
...
BEGIN
    ...
    ORDER BY 10 DESC, o.monto DESC
    LIMIT COALESCE(NULLIF(p_limit, 0), 5);
END;
$$;

-- =============================================================================
-- 9.1 SEED: matches y decision minima para demo estable
-- =============================================================================
INSERT INTO match_oportunidades (productor_id, oportunidad_id, score, estado, datos_match)
SELECT
    p.id,
    m.oportunidad_id,
    m.score,
    ...
    jsonb_build_object(
        ...
        'monto', m.monto,
        'rubro_requerido', m.rubro_requerido,
        'unspsc_code', m.unspsc_code,
        'url_fuente', m.url_fuente,
        'fuente', 'matching_oportunidades'
    )
FROM productores p
CROSS JOIN LATERAL matching_oportunidades(p.id, 5) m
WHERE p.nombre = 'Vilma Jeanneth Guardado de Ayala'
ON CONFLICT (productor_id, oportunidad_id) DO UPDATE SET
    score = EXCLUDED.score,
    estado = EXCLUDED.estado,
    datos_match = EXCLUDED.datos_match;
...
SELECT
    p.id,
    m.oportunidad_id,
    'Me interesa participar, pero me preocupa si puedo cumplir con el volumen y esperar el pago del Estado.',
    'demo-vilma-fase-1',
    '["Ya calculo cuanto le cuesta producir cada almuerzo con los precios actuales?", "Tiene flujo de caja para aguantar 60 a 90 dias antes del pago?", "Que plan B tiene si gana y no puede cumplir con la cantidad completa?"]'::jsonb,
    '["Ya calculo cuanto le cuesta producir cada almuerzo con los precios actuales?", "Tiene flujo de caja para aguantar 60 a 90 dias antes del pago?", "Que plan B tiene si gana y no puede cumplir con la cantidad completa?"]'::jsonb,
    'Demo Fase 1: preguntas enfocadas en costo unitario, flujo de caja y riesgo operativo.',
    '{"source":"seed-demo-fase-1","fallback":false}'::jsonb,
    false,
    now()
FROM productores p
JOIN LATERAL matching_oportunidades(p.id, 1) m ON true
WHERE p.nombre = 'Vilma Jeanneth Guardado de Ayala'
  AND NOT EXISTS (SELECT 1 FROM decisiones d WHERE d.conversation_id = 'demo-vilma-fase-1');
-- =============================================================================
-- 10. VERIFICACIÓN
-- =============================================================================
...
DROP POLICY IF EXISTS "plantillas_documentos_all_access" ON plantillas_documentos;
CREATE POLICY "plantillas_documentos_all_access" ON plantillas_documentos FOR ALL USING (true) WITH CHECK (true);

-- 11.2 Auditoría de documentos generados
CREATE TABLE IF NOT EXISTS documentos_generados (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    productor_id    UUID REFERENCES productores(id) ON DELETE CASCADE,
    plantilla_id    UUID REFERENCES plantillas_documentos(id) ON DELETE SET NULL,
...
DROP POLICY IF EXISTS "documentos_generados_all_access" ON documentos_generados;
CREATE POLICY "documentos_generados_all_access" ON documentos_generados FOR ALL USING (true) WITH CHECK (true);
...
(
    ...
    'DocumentacionLegal/Documentación-usuario/Documentación-usuario/PERSONA-NATURAL.pdf'
),
(
    ...
    'DocumentacionLegal/Documentación-usuario/Documentación-usuario/PERSONA-NATURAL-APODERADO.pdf'
),
(
    ...
    'DocumentacionLegal/Documentación-usuario/Documentación-usuario/PERSONA-JURIDICA.pdf'
),
(
    ...
    'DocumentacionLegal/Documentación-usuario/Documentación-usuario/PERSONA-JURIDICA-APODERADO.pdf'
),
(
    ...
    'DocumentacionLegal/Documentación-usuario/Documentación-usuario/DES-2025-001-Documento-Estandar-de-contratacion-directa-version-word.docx'
),
(
    'Documento Estándar de Comparación de Precios - Bienes y Servicios',
    'documento_estandar_comparacion_precios_bienes_servicios',
    '["nombre","dui","nit","direccion","oportunidad_titulo","institucion","monto","items","precios_unitarios","fecha"]'::jsonb,
    'https://dinac.gob.sv/documentos/comparacion-precios-bienes-servicios',
    'DocumentacionLegal/Documentación-usuario/Documentación-usuario/DES-2024-004.-Documento-Estandar-Comparacion-de-Precios.-Bienes-y-Servicios.docx'
),
(
...
(
    ...
    'desglose_precios',
    '["nombre","nit","oportunidad_titulo","items","precios_unitarios","fecha"]'::jsonb,
