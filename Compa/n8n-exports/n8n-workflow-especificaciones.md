...
// IDs reales de Supabase. No usar nombre_productor ni conversation_id como FK.
const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const productorId = payload.productor_id;
const oportunidadId = payload.oportunidad_id;

if (!uuidRegex.test(productorId || '')) {
  throw new Error('Validation error: productor_id must be a Supabase UUID');
}

if (!uuidRegex.test(oportunidadId || '')) {
  throw new Error('Validation error: oportunidad_id must be a Supabase UUID');
}
...
// Contexto de oportunidad. Para demo debe venir del match seleccionado en Supabase.
...
return [{
  json: {
    ...
    productor_id: inputData[0]?.productor_id,
    oportunidad_id: inputData[0]?.oportunidad_id,
